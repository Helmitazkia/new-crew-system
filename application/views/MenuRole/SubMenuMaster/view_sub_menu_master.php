        ...
        <div class="card shadow">
          <div class="card-body">
            ...
            <div class="table-responsive">
              <table id="subMenuTable" class="table table-bordered align-middle mb-0 crew-table" style="width:100%">
                <thead class="crew-header">
                  <tr>
                    <th class="text-center" style="width:50px;">No</th>
                    <th>Parent Menu <span class="filter-icon">☰</span></th>
                    <th>Sub Menu Code <span class="filter-icon">☰</span></th>
                    <th>Sub Menu Name <span class="filter-icon">☰</span></th>
...
<!-- Modal -->
<div class="modal fade" id="subMenuModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
    ...
    <div class="modal-content border-0 shadow">
      <div class="modal-header" style="background: linear-gradient(135deg, #000099 0%, #1a237e 100%); color:#fff;">
        <h6 class="modal-title fw-bold" id="modalTitle">
          <i class="fa fa-bars me-2"></i>Add Sub Menu
        </h6>
            ...
            <form id="subMenuForm">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control selectpicker" data-live-search="true" title="- Select Parent Menu -" name="menuId" id="menuId" required>
                            <?php foreach($parents as $parent): ?>
                                <option value="<?php echo $parent->menuId ?>"><?php echo $parent->menuName ?> (<?php echo $parent->menuCode ?>)</option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-semibold">Sub Menu Code <span class="text-danger">*</span></label>
                            <input type="text" class="form-control form-control-sm" name="subMenuCode" id="subMenuCode" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-semibold">Sub Menu Name <span class="text-danger">*</span></label>
                            <input type="text" class="form-control form-control-sm" name="subMenuName" id="subMenuName" required>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-8 mb-3">
                            <label class="form-label fw-semibold">URL Controller <span class="text-danger">*</span></label>
                            <input type="text" class="form-control form-control-sm" name="subMenuUrl" id="subMenuUrl" required>
                        </div>
                        <div class="col-md-4 mb-3">
                            <label class="form-label fw-semibold">Order <span class="text-danger">*</span></label>
                            <input type="number" class="form-control form-control-sm" name="subMenuOrder" id="subMenuOrder" value="0" required>
                        </div>
                    </div>
                    <div class="mb-3">
...
$(document).ready(function() {
    ...
    table = $('#subMenuTable').DataTable({
        ...
        ajax: {
            url: '<?php echo base_url("MenuRole/SubMenuMaster/getAllData") ?>',
        ...
        columns: [
            ...
            { data: 'subMenuName' },
            { data: 'subMenuUrl' },
            { data: 'subMenuOrder', className: 'text-center' },
            ...
            { data: 'subMenuId', className: 'text-center', render: function(data) {
    ...
    // Column search
    $('#subMenuTable thead tr:eq(1) .column-search').on('keyup change', function() {
    ...
    $('#subMenuForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '<?php echo base_url("MenuRole/SubMenuMaster/save") ?>',
            type: 'POST',
            data: $(this).serialize(),
            dataType: 'json',
...
function initDropdownFilters(api) {
    $('#subMenuTable thead th').each(function (colIndex) {
        var icon = $(this).find('.filter-icon');
        if (!icon.length) return;
        if (colIndex === 0 || colIndex === 7) return; // skip No & Action
...
function editData(id) {
    $.post('<?php echo base_url("MenuRole/SubMenuMaster/getById") ?>', {id: id}, function(res) {
        if (res.status == 'success') {
            $('#subMenuForm')[0].reset();
            ...
            $('#subMenuOrder').val(res.data.subMenuOrder);
...
function deleteData(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.post('<?php echo base_url("MenuRole/SubMenuMaster/delete") ?>', {id: id}, function(res) {
                if(res.status == 'success') {
                    table.ajax.reload();
