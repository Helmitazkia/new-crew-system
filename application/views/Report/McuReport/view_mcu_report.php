                                ...
                                <thead style="background-color: #e8eaf6;">
                                    <tr>
                                        ...
                                        <th class="text-center" style="width: 50px;">
                                            <button type="button" class="btn btn-sm btn-success" id="btnAddCrewRow"><i class="fa fa-plus"></i></button>
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
...
$(document).ready(function() {
    ...
    var BASE_URL = '<?php echo base_url("Report/McuReport"); ?>';
   ...
   // console.log(idperson);
    ...
    $('#btnAddCrewRow').on('click', function() {
        var rowHtml = '<tr>' +
            '<td><select class="form-control select2-crew" style="width:100%;"></select></td>' +
            '<td><input type="text" class="form-control form-control-sm crew-rank" placeholder="Jabatan" required></td>' +
            '<td><input type="text" class="form-control form-control-sm crew-vessel" placeholder="Kapal" required></td>' +
            '<td class="text-center"><button type="button" class="btn btn-sm btn-danger btn-remove-crew"><i class="fa fa-trash"></i></button></td>' +
            '</tr>';
        
        var $row = $(rowHtml);
        $('#tblAddCrewList tbody').append($row);
        ...
        $row.find('.select2-crew').select2({
            ...
            $.ajax({
                ...
                type: 'POST',
                data: { idperson: data.id },
                dataType: 'json',
                success: function(res) {
                    if(res.success && res.data) {
                        $currRow.find('.crew-rank').val(res.data.jabatan || '');
                        $currRow.find('.crew-vessel').val(res.data.vessel_name || '');
                    }
                }
            });
        });
    ...
    $('#formAddMcu').on('submit', function(e) {
        ...
        var crewList = [];
        $('#tblAddCrewList tbody tr').each(function() {
            var idPerson = $(this).find('.select2-crew').val();
            var nameCrew = $(this).find('.select2-crew option:selected').text();
            var rank = $(this).find('.crew-rank').val();
            var vessel = $(this).find('.crew-vessel').val();
        ...
        if (crewList.length === 0) {
            ...
            showNotification('warning', 'Minimal pilih 1 crew!');
            return;
        }
