        ...
        <div class="card shadow">
          <div class="card-body">
            ...
            <div class="d-flex justify-content-end align-items-center mb-3">
              <!-- <h5 class="m-0 fw-bold" style="color: #000099;">Master Familiarization Topic</h5> -->
              <button class="btn btn-sm fst-italic fw-semibold shadow-sm"
                      onclick="showAddModal()"
                      style="background-color:#000099; color:#fff; border-color:#000099;">
                <i class="fa fa-plus me-1"></i> Tambah Topic
              </button>
            </div>
            <!-- DataTable -->
            <div class="table-responsive">
              <table id="topicTable" class="table table-bordered align-middle mb-0 crew-table" style="width:100%">
...
<!-- Modal -->
<div class="modal fade" id="topicModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
    ...
    <div class="modal-content border-0 shadow">
      <div class="modal-header" style="background: linear-gradient(135deg, #000099 0%, #1a237e 100%); color:#fff;">
        <h6 class="modal-title fw-bold" id="modalTitle">
          <i class="fa fa-list me-2"></i>Add Topic
        </h6>
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control form-control-sm" name="orderNo" id="orderNo" value="0" required>
                        </div>
                        <div class="col-md-6 mb-3">
...
$(document).ready(function() {
    table = $('#topicTable').DataTable({
        ...
        ajax: {
            url: '<?php echo base_url("MasterData/MasterFamiliarization/getAllDataTopic") ?>',
    ...
    // Column search
    $('#topicTable thead tr:eq(1) .column-search').on('keyup change', function() {
    ...
    $('#topicForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '<?php echo base_url("MasterData/MasterFamiliarization/saveTopic") ?>',
            type: 'POST',
            data: $(this).serialize(),
            dataType: 'json',
...
function initDropdownFilters(api) {
    $('#topicTable thead th').each(function (colIndex) {
        ...
        if (colIndex === 0 || colIndex === 4) return; // skip No & Action
...
function editData(id) {
    $.post('<?php echo base_url("MasterData/MasterFamiliarization/getTopicById") ?>', {id: id}, function(res) {
        if (res.status == 'success') {
            $('#topicForm')[0].reset();
            $('#topicId').val(res.data.id);
            $('#topicName').val(res.data.topic_name);
...
function deleteData(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.post('<?php echo base_url("MasterData/MasterFamiliarization/deleteTopic") ?>', {id: id}, function(res) {
                if(res.status == 'success') {
                    table.ajax.reload();
