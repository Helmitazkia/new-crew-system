...
class Pkl extends CI_Controller {
    ...
    {
        $this->load->view('ListReport/Pkl/view_pkl_new');
    }
    ...
    function get_history()
    {
        ...
        $this->db->from('history_pkl_wages h');
    ...
    function PrintPKL($id = "")
    {
        ...
        if ($isHistory) {
            ...
            // Execute the exact query structure provided by the user
            $sql = "
                SELECT 
                    CONCAT_WS(' ', p.fname, p.mname, p.lname) AS fullname,
                    p.dob,
                    k.NmKota AS pob,
                    p.idperson,
                    p.nationalid AS nationality,
            ...
            $sqlWages = "
                ...
                FROM wages_pkl
                WHERE idperson = '".$id."'
                AND saltype IN ('001','006','008','005')
                AND deletests = 0
            ";

            $salaryData = $this->MCrewscv->getDataQuery($sqlWages);

            $basic = $fix = $leave = $tanker = 0;

            if (!empty($salaryData)) {
                foreach ($salaryData as $s) {
                    if ($s->saltype == '001') $basic  = (int)$s->amount;
                    if ($s->saltype == '006') $fix    = (int)$s->amount;
                    if ($s->saltype == '008') $leave  = (int)$s->amount;
                    if ($s->saltype == '005') $tanker = (int)$s->amount;
                }
            }

            $total = $basic + $fix + $leave + $tanker;
        ...
        ob_start();
        $this->load->view('ListReport/Pkl/form_pkl_pdf', $dataOut);
        $html = ob_get_contents();
        ob_end_clean();

        $mpdf->WriteHTML($html);
        $mpdf->Output("PKL_" . $dataOut['crew']->fullname . ".pdf", 'I');
        exit;
    } 
    ...
    function saveVesselData()
    {
        ...
        $insert = $this->db->insert('history_pkl_wages', $insertData);
